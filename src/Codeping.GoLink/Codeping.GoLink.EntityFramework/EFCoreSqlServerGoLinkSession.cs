...
{
    internal class EFCoreSqlServerGoLinkSession : IGoLinkSession
    {
        private readonly ILogger _logger;
        private readonly GoLinkDbContext _context;

        public EFCoreSqlServerGoLinkSession(IServiceProvider provider)
        {
            _logger = provider.GetService<ILogger<EFCoreSqlServerGoLinkSession>>();

            var options = provider.GetService<DbContextOptions>();

            if (options != null && !options.IsFrozen && options.ContextType != null)
            {
                _context = provider.GetService(options.ContextType).Cast<GoLinkDbContext>();
            }
